...
**2. 温控 QTE 机制 (Thermodynamics)**
烹饪不再是瞬间完成，而是一场对**温度**的精准把控游戏。
*   **升温**:
    *   下方热源持续提供基础热量。
    *   使用 **蒲扇 (Hand Fan)** 右键扇风，可短时大幅提升火力。
*   **降温**:
    *   使用 **木汤勺 (Wooden Ladle)** 右键搅拌，防止糊锅并微量降温。
    *   直接倒入 **水桶/水瓶**，急剧降温（防止烧焦）。
*   **最佳温区 (Optimal Zone)**:
    *   每道菜谱都有特定的“最佳温度范围”。
    *   锅具上方会显示 **实时温度条** (World-Space UI)。
    *   **QTE 目标**: 尽可能让温度保持在绿色/最佳区域内。

**3. 品质结算 (Quality & Result)**
最终成品的品质由**食材品质**与**厨艺 (QTE)** 共同决定。
*   **公式**: `FinalStars = (食材平均星级 * 50%) + (QTE评分 * 50%)`
    *   **食材分**: 取决于投入作物的基因等级 (1-5星)。
    *   **QTE分**: 取决于烹饪过程中温度停留在最佳温区的时间比例 (0% - 100%)。
*   **结果**:
    *   **成功**: 获得带有 PDC 星级数据的美味佳肴，星级越高，食用 Buff 越强。
    *   **失败**: 若配方错误或QTE极差（烧焦），将产出 **黑暗料理 (Mystery Stew)**。
