...
## 1. 系统概述 (Overview)
**耕食记** 是一个深度整合农业种植与烹饪制作的沉浸式经营插件。目前阶段核心实现了 **动态肥力系统**、**孟德尔基因系统** 与 **沉浸式烹饪系统**，完整构建了“播种-收获-烹饪-食用”的玩法闭环。玩家不仅需要维护土地肥力、选育优良基因，还需要在烹饪过程中通过温控与搅拌制作出具有强大增益的美食。
...
| **烹饪核心** | `CookingManager.java` | **[核心]** 负责烹饪配方的注册、初始化与管理。 |
| **锅具实体** | `CookingPot.java` | **[核心]** 封装单个炼药锅的物理状态、温度模拟、QTE判定与视觉效果。 |
| **锅具界面** | `CookingPotInventory.java` | 管理 54 格烹饪 GUI 的布局、交互限制与数据同步。 |
| **烹饪交互** | `CookingPotListener.java` | 监听玩家与炼药锅的交互（打开界面、使用工具、添加食材）。 |
| **食用效果** | `FoodConsumeListener.java` | 监听玩家食用自定义美食，应用特殊的回复效果与 Buff。 |
...
### 3.9 遗传分析仪 (Genetic Analyzer)
*(Updated 2025-12-26)*

该设备的功能已从单纯的种子分析扩展至通用的基因检测。不仅可以鉴定未知种子的基因，还兼容 **畜牧系统 (PastureSong)** 的 DNA 样本分析。

*   **机器**: `GENETIC_ANALYZER` (遗传分析仪)
    *   *原名: 种子分析仪 (Seed Analyzer)*
    *   外观: 铁块材质的自定义方块。
    *   获取: `/getanalyzer` (管理员) 或 合成 (待定)。
*   **使用流程**:
    1.  放置分析仪并右键打开 GUI。
    2.  在左侧插槽放入 **未鉴定种子** 或 **DNA 样本**。
    3.  点击中间的 **绿色按钮** 开始分析。
    4.  等待分析完成（模拟运算过程）。
    5.  从右侧插槽取出 **已鉴定物品**。
*   **鉴定结果**:
    *   **种子**: Lore 中展示 6 对等位基因及性状评分、星级。
    *   **DNA 样本**: Lore 中展示 4 对等位基因（生长势/品质/抗性/多产性）及性状描述。
...
*   **启动/降温**: 使用 **木汤勺 (Wooden Ladle)** 右键搅拌锅具可 **激活烹饪状态** (需温度达标) 或在过热时降温。直接倒入 **水/冰** 可大幅降温。
*   **自适应 UI (Adaptive UI)**:
...
*   **结果分支**:
    ...
    *   **黑暗料理 (Mystery Stew)**: 
        *   触发条件：放入了无法匹配任何食谱的食材组合（或长时间未搅拌导致糊锅）。
        *   惩罚机制：烹饪结束后，锅内**每种食材扣除 1 个**，并产出一碗带有反胃效果的黑暗料理。

### 3.11 食谱分级与食用效果 (Recipe Tiers & Consumption)
*(New 2025-12-24)*

为了构建差异化的饮食体验，系统将食谱划分为三个核心层级，并引入了基于 **星级 (Star Rating)** 的动态效果倍率。

#### 3.11.1 星级效果 (Star Rating Effects)
食物的效果不再固定，而是根据烹饪出的星级进行修正：
*   **1⭐ (Dark Food)**: 烹饪失败或极差品质。转化为 **黑暗料理**，失去原有效果并带有负面 Debuff。
*   **2⭐ (Poor)**: **70%** 基础效果。
*   **3⭐ (Standard)**: **100%** 基础效果。
*   **4⭐ (High)**: **130%** 基础效果。
*   **5⭐ (Legendary)**: **200%** 基础效果，且附带 **专属 Lore 描述** 以彰显尊贵。

#### 3.11.2 食谱层级 (Tiers)

**Tier 1: 饱食 (Sustenance)**
*   **定位**: 基础生存，高性价比。
*   **特点**: 成本低廉，提供极高的 **饱食度 (Food Level)** 与 **饱和度 (Saturation)** 回复。
*   **代表**: `Simple Stew` (家常炖肉) - 回复 16点饱食度 + 20.0 饱和度 (3星基准)。

**Tier 2: 效能 (Utility)**
*   **定位**: 功能性饮品/料理。
*   **特点**: 提供中短期的 **Potion Buff**，星级影响持续时间。
*   **代表**: `Miner's Tonic` (矿工特饮) - 给予急迫 (Haste) 与夜视效果。

**Tier 3: 珍馐 (Delicacy)**
*   **定位**: 顶级料理，永久性提升。
*   **特点**: 制作繁琐，但能提供 **永久性收益** 或 **特殊资源**。
*   **代表**: `Wisdom Broth` (智慧浓汤)
    *   **机制**: 饮用后获得 **随机经验值 (XP)**。
    *   **梯度**:
        *   **Low (20-50 XP)**: 常见。
        *   **Medium (50-100 XP)**: 一般。
        *   **High (100-200 XP)**: 稀有。
        *   **Gold (2000 XP)**: **传说级欧皇时刻** (基准概率约 0.26%，即 CS2 开箱出金概率)。触发时会有全服通告与特效。
    *   **星级加成**: 5星品质的浓汤将“出金”概率提升至约 **1%**。

**技术实现 (`FoodConsumeListener`)**:
*   系统监听玩家食用带有特定 `CustomModelData` 的食物。
*   优先读取 ItemMeta 中的 `star_rating` (PDC)，计算倍率。
*   对于 T3 食物，采用加权随机算法计算收益。
...
| `/getfan` | `cuisinefarming.get` | 获取蒲扇 (烹饪升温)。 |
| `/getladle` | `cuisinefarming.get` | 获取木汤勺 (烹饪搅拌)。 |
| `/getpot` | `cuisinefarming.get` | 获取厨锅。 |
